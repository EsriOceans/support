...
dbf_path = os.path.join(test_location, '{}.dbf'.format(table_name))
arcpy.TableToDBASE_conversion(table_path, test_location)

print("testing FGDB driver")
try:
...
# try the same thing with a shapefile backed datasource
print("testing DBF driver")
try:
    with arcpy.da.SearchCursor(dbf_path, fouled_up_fields) as cursor:
        for row in cursor:
            continue
except RuntimeError as e:
    print("We've now hit a runtime error, due to the missing column " + \
